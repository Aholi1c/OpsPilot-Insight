...
_CATEGORY_KEYWORDS = {
    ...
    "transaction_risk_surge": [
        "transaction_risk_surge", "AbnormalTransactionSurge", "risk_score",
        "异常交易", "撞库", "风控", "冻结",
    ],
}
...
_TEMPLATES = {
    ...
    ("incident_summary", "transaction_risk_surge"):
        "risk-engine 检测到异常交易激增：高风险交易判定由 2 笔/分钟飙升至 187 笔/分钟，"
        "47 个账户命中撞库盗刷特征，payment-gateway 交易成功率跌至 81.4%，"
        "涉及客户资金安全，属 P1 级资金风险事件。",
    ...
    ("rca_analysis", "network_latency"):
        ...
        "建议回滚该网络变更并联动网络团队核查。",
    ("rca_analysis", "transaction_risk_surge"):
        "综合日志、指标、链路与变更四个维度：风控引擎持续输出高危评分（risk_score>0.9）"
        "的可疑交易判定，命中账户呈现『多次登录失败后成功登录、设备指纹不一致、"
        "批量大额转出至新增收款人』的典型撞库盗刷序列；时间窗内无内部风控规则变更，"
        "排除误报放大假设，判定为撞库攻击导致批量账号被盗，建议立即冻结可疑账户止损。",
    ...
    ("plan_narrative", "transaction_risk_surge"):
        "建议立即批量冻结命中撞库特征的可疑账户以阻断资金转出，同时对设备指纹异常"
        "会话强制下线并要求二次验证，通知风控值班与反欺诈团队人工核验冻结名单；"
        "误冻结账户经人工核验后批量解冻，全程观察交易成功率与欺诈告警量回归基线。",
    ...
    ("postmortem_summary", "transaction_risk_surge"):
        "本次事件由撞库攻击导致批量账号被盗触发异常交易激增，处置链路为批量冻结"
        "可疑账户止损、高风险会话强制二次验证、通知风控与反欺诈团队人工核验，"
        "处置后欺诈告警量与交易成功率均回归基线，未发生资金损失外溢；后续需在"
        "登录链路增加撞库特征限流与设备指纹强校验，并建立账户冻结/解冻标准预案。",
}
